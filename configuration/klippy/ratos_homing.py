        ...
        try:
            ...
            if self.z_hop != 0.0:
                ...
                if 'z' not in kin_status['homed_axes']:
                    ...
                    pos[2] = 0
                    # RatOS-Kalico: Kalico's contract is axis names, not indices.
                    toolhead.set_position(pos, homing_axes="z")
                    toolhead.manual_move([None, None, self.z_hop],
                                        self.z_hop_speed)
                    # RatOS-Kalico: prefer Kalico's clear_homing_state, fall back
            ...
            if bed_mesh is not None and bed_mesh.z_mesh is not None:
                raise self.printer.command_error(f"Homing may not be possible when a bed mesh is loaded. Clear the bed mesh and try again. {e}") from e
            raise e
